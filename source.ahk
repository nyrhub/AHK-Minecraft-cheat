; I can't remember who does the GUI , but thanks to him ! Memory class by RHCP ! The rest by nyr!
; I don't usually do AHK but being one of the first in the world to suggest it was so tempting
; If you want to take orders for anything in c# feel free to contact me : https://github.com/nyrhub !
...
Application := {Name: "Minecraft cheat in autohotkey - for 1.8.9", Version: "1.0"}
Window := {Width: 500, Height: 250, Title: Application.Name}
Navigation := {Label: ["AutoClicker", "HitBox", "AirJump", "SpeedHack", "---", "Misc", "Credits"]}
...
Gui, Add, Button, x425 y60 w50 gACBind vbindac, bind
Gui, Add, Button, x175 y100 w300 h125 gAConoff vAConoff, Off

Gui, Tab, 2 ; Future controls are owned by the specified tab
Gui, Add, Button, x425 y60 w50 gHBBind vbindhb, bind
Gui, Add, Button, x175 y100 w300 h125 gHBonoff vHBonoff, Off

Gui, Tab, 3 ; Future controls are owned by the specified tab
Gui, Add, Button, x425 y60 w50 gAJBind vbindaj, bind
Gui, Add, Button, x175 y100 w300 h125 gAJonoff vAJonoff, Off

Gui, Tab, 4 ; Future controls are owned by the specified tab
Gui, Add, Button, x425 y60 w50 gSHBind vbindsh, bind
Gui, Add, Button, x175 y100 w300 h125 gSHonoff vSHonoff, Off

Gui, Tab, 5 ; Future controls are owned by the specified tab
; Skipped

Gui, Tab, 6 ; Future controls are owned by the specified tab
Gui, Add, Button, x175 y100 w300 h125 gsd, Self-Destruct

Gui, Tab, 7 ; Future controls are owned by the specified tab
Gui, Add, Text, x125 y75, Made by nyr
Gui, Add, Text, x125 y100, Memory class by RHCP
Gui, Add, Text, x125 y125, I can't remember who does the GUI
Gui, Font, s6
Gui, Add, Text, x125 y150, Pointers can probably be skipped which would make some functions unusable
Gui, Add, Text, x125 y175, I really did something very minimal, if you want to improve it, download the source code on my github
Gui, Font, s9
Gui, Add, Button, x125 y190 w200 h50 gGit, GitHub
...
git:
Run, https://github.com/nyrhub/AHK-Minecraft-cheat
return
...
AConoff:
GuiControlGet, AConoff
if (AConoff = "Off")
{
GuiControl,, AConoff, On
SetTimer, AC, 100
return
}
if (AConoff = "On")
{
GuiControl,, AConoff, Off
SetTimer, AC, Off
return
}

HBonoff:
GuiControlGet, HBonoff
java := new _ClassMemory("ahk_exe javaw.exe")
if !isObject(java)
    msgbox minecraft is not running
	   baseAddress := java.getModuleBaseAddress("jvm.dll")
arrayPointerOffsets := [0x598, 0x180, 0x88, 0x810, 0x218, 0xD8, 0x4E0]
value := java.read(baseAddress + 0x0005B5B8, "Float", arrayPointerOffsets*)
if (HBonoff = "Off")
{
GuiControl,, HBonoff, On
   java.write(baseAddress + 0x0005B5B8, 3, "Float", arrayPointerOffsets*)
return
}
if (HBonoff = "On")
{
GuiControl,, HBonoff, Off
java.write(baseAddress + 0x0005B5B8, 0.6, "Float", arrayPointerOffsets*)
return
}

AJonoff:
GuiControlGet, AJonoff
java := new _ClassMemory("ahk_exe javaw.exe")
if !isObject(java)
    msgbox minecraft is not running
baseAddress := java.getModuleBaseAddress("jvm.dll")
arrayPointerOffsets := [0x200, 0x160, 0xA80, 0x844, 0x18, 0x22C, 0xF0]
value := java.read(baseAddress + 0x007E17B0, "Int", arrayPointerOffsets*)
if (AJonoff = "Off")
{
GuiControl,, AJonoff, On
SetTimer, AJt, 75
return
}
if (AJonoff = "On")
{
GuiControl,, AJonoff, Off
SetTimer, AJt, Off
java.write(baseAddress + 0x007E17B0, 65537, "Int", arrayPointerOffsets*)
return
}

AC:
While (WinActive("Minecraft 1.8.9")){
    Click
	sleep, 100
}
...
arrayPointerOffsets := [0x200, 0x160, 0xA80, 0x844, 0x18, 0x22C, 0xF0]
value := java.read(baseAddress + 0x007E17B0, "Int", arrayPointerOffsets*)
java.write(baseAddress + 0x007E17B0, 65538, "Int", arrayPointerOffsets*)
return

SHonoff:
GuiControlGet, SHonoff
java := new _ClassMemory("ahk_exe javaw.exe")
if !isObject(java)
    msgbox minecraft is not running
baseAddress := java.getModuleBaseAddress("jvm.dll")
arrayPointerOffsets := [0x578, 0xE0, 0x378, 0x0, 0x244, 0xAC, 0x510]
value := java.read(baseAddress + 0x0005B5B8, "Double", arrayPointerOffsets*)
if (SHonoff = "Off")
{
GuiControl,, SHonoff, On
java.write(baseAddress + 0x0005B5B8, 0.9, "Double", arrayPointerOffsets*)
return
}
if (SHonoff = "On")
{
GuiControl,, SHonoff, Off
java.write(baseAddress + 0x0005B5B8, 0.1, "Double", arrayPointerOffsets*)
return
}


ACBind: ; I did not make this option I was too lazy sorry
GuiControlGet, bindac
GuiControl,, bindac, ...
loop
{
        Input, pressedKey, L1 E, {LControl}{RControl}{LAlt}{RAlt}{LShift}{RShift}{LWin}{RWin}{AppsKey}{F1}{F2}{F3}{F4}{F5}{F6}{F7}{F8}{F9}{F10}{F11}{F12}{Left}{Right}{Up}{Down}{Home}{End}{PgUp}{PgDn}{Del}{Ins}{BS}{CapsLock}{NumLock}{PrintScreen}{Pause} ; ...
        GuiControl,, bindac, %pressedKey%
		return
}	

HBBind: ; I did not make this option I was too lazy sorry
GuiControlGet, bindhb
GuiControl,, bindhb, ...
loop
{
        Input, pressedKey, L1 E, {LControl}{RControl}{LAlt}{RAlt}{LShift}{RShift}{LWin}{RWin}{AppsKey}{F1}{F2}{F3}{F4}{F5}{F6}{F7}{F8}{F9}{F10}{F11}{F12}{Left}{Right}{Up}{Down}{Home}{End}{PgUp}{PgDn}{Del}{Ins}{BS}{CapsLock}{NumLock}{PrintScreen}{Pause} ; ...
        GuiControl,, bindhb, %pressedKey%
		GuiControl, Disable, HBBind
	return
}

AJBind: ; I did not make this option I was too lazy sorry
GuiControlGet, bindaj
GuiControl,, bindaj, ...
loop
{
        Input, pressedKey, L1 E, {LControl}{RControl}{LAlt}{RAlt}{LShift}{RShift}{LWin}{RWin}{AppsKey}{F1}{F2}{F3}{F4}{F5}{F6}{F7}{F8}{F9}{F10}{F11}{F12}{Left}{Right}{Up}{Down}{Home}{End}{PgUp}{PgDn}{Del}{Ins}{BS}{CapsLock}{NumLock}{PrintScreen}{Pause} ; ...
        GuiControl,, bindaj, %pressedKey%
		GuiControl, Disable, AJBind
	return
}

SHBind: ; I did not make this option I was too lazy sorry
GuiControlGet, bindsh
GuiControl,, bindsh, ...
loop
{
        Input, pressedKey, L1 E, {LControl}{RControl}{LAlt}{RAlt}{LShift}{RShift}{LWin}{RWin}{AppsKey}{F1}{F2}{F3}{F4}{F5}{F6}{F7}{F8}{F9}{F10}{F11}{F12}{Left}{Right}{Up}{Down}{Home}{End}{PgUp}{PgDn}{Del}{Ins}{BS}{CapsLock}{NumLock}{PrintScreen}{Pause} ; ...
        GuiControl,, bindsh, %pressedKey%
		GuiControl, Disable, SHBind
	return
}
